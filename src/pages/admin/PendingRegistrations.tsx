          ...
          if (membership) {
            ...
            const membershipData = {
              ...
              description: membership.description || '',
               paymentStatus: 'pending',  // ✅ AGREGAR ESTA LÍNEA
               currentAttendances: 0,      // ✅ AGREGAR ESTA LÍNEA
              createdAt: serverTimestamp(),
              updatedAt: serverTimestamp()
            };

            // Guardar la membresía
const membershipRefCreated = await addDoc(
  collection(db, `gyms/${gymData.id}/members/${newMemberId}/memberships`),
  membershipData
);
const membershipIdCreated = membershipRefCreated.id;

// 🔧 CORRECCIÓN: Generar el pago pendiente en la colección correcta
const currentMonth = new Date();
const year = currentMonth.getFullYear();
const month = String(currentMonth.getMonth() + 1).padStart(2, '0');
const monthStr = `${year}-${month}`;
const dueDate = `${monthStr}-15`; // Vence siempre el día 15

const paymentId = `${monthStr}-${newMemberId}-${membershipIdCreated}`;
const paymentData = {
  memberId: newMemberId,
  memberName: `${registration.firstName} ${registration.lastName}`,
  membershipId: membershipIdCreated,
  activityId: membership.activityId,
  activityName: membership.activityName,
  month: monthStr,
  dueDate: dueDate,
  ...
  createdBy: userData.name
        ...
        await setDoc(
          doc(db, `gyms/${gymData.id}/monthlyPayments`, paymentId),
          paymentData
        );

        console.log('✅ Pago mensual creado:', {
          paymentId,
